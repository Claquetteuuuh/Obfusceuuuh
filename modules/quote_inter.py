...
def interrupt(payload: str):
    ...
    crochet = 0
    separator = [" ", ";", "'", '"', "`", "=", ","]
    payload_divided_for = payload.split("for")
    for k in range(len(payload_divided_for)):
        payload_divided_if = payload_divided_for[k].split("if")
        for j in range(len(payload_divided_if)):
            ...
            if j < len(payload_divided_if) -1:
                fnl += "if"
        if k < len(payload_divided_for) -1:
            fnl += "for"
    return fnl
